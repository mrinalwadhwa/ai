...
Reusable testing patterns live in `testing/` as individual files. Index each one below with a single-line load trigger so an agent reads a pattern only when its trigger applies. Add a `testing/<subarea>/` directory and a subarea file (with its own `## Patterns`) when a narrower theme accumulates enough material to warrant scoping.

- [asserting-on-generated-output](testing/generated-output.md) — read when asserting on output from template rendering, codegen, prompt generation, or any data-to-text transformation.
- [comparing-structured-data](testing/structured-data.md) — read when asserting on the content of a JSON/YAML/TOML file or any structured-data format that has multiple valid serializations.
